...
const TablaInventario = ({ insumos, insumosEnUso = new Map(), loading, filaSeleccionada, setFilaSeleccionada, abrirModalEdicion, onEliminar, gridLayout }) => {
                  ...
                  <div className="flex items-center justify-center p-3">
                    {esInsumo ? (
                      <div className="flex items-center gap-3">
                        {/* IN */}
                        <button type="button" className="hover:scale-110 transition-transform cursor-pointer"
                    ...
                    <div className="flex justify-between items-start mb-3">
                      ...
                      {esInsumo ? (
                        <div className="flex items-center gap-3">
                          ...
                          {/* ELIMINAR */}
                          {enUso ? (
                            <button type="button" onClick={(e) => { e.stopPropagation(); setMensajeInfo(MENSAJE_INFO_EN_USO(tipoEnUso)); setMostrarInfoNoEditable(true); }}>
                              <HugeiconsIcon icon={InformationCircleIcon} size={22} color={colores.azul} />
                            </button>
                          ) : (
                            <button type="button" onClick={(e) => { e.stopPropagation(); onEliminar(item); }}>
