...
import TablaBloques from '../../../features/lotes/components/TablaBloques'
...
const bloquesMock = [
    { id_bloque: 1, peso_gr: '850.5', contenedor: 'Bolsa', produccion: 1, contaminado: 0 },
    { id_bloque: 2, peso_gr: '920.0', contenedor: 'Frasco', produccion: 0, contaminado: 1 },
    { id_bloque: 3, peso_gr: '0', contenedor: 'Bolsa', produccion: true, contaminado: false },
]
...
describe('TablaBloques — renderizado base', () => {
    ...
    it('muestra el encabezado "Código del bloque"', () => {
        renderComponente()
        expect(screen.getAllByText('Código del bloque').length).toBeGreaterThan(0)
    })
    ...
    it('muestra el encabezado "Peso (g)"', () => {
        renderComponente()
        expect(screen.getAllByText('Peso (g)').length).toBeGreaterThan(0)
    })
...
describe('TablaBloques — contenedor y peso', () => {
    ...
    it('muestra el peso redondeado al entero', () => {
        renderComponente()
        // 850.5 → "851 g", 920.0 → "920 g", 0 → "0 g"
        expect(screen.getAllByText('851 g').length).toBeGreaterThan(0)
        expect(screen.getAllByText('920 g').length).toBeGreaterThan(0)
        expect(screen.getAllByText('0 g').length).toBeGreaterThan(0)
    })

    it('muestra "0 g" cuando peso_gr es falsy', () => {
        renderComponente({
...
describe('TablaBloques — clasificación', () => {
    ...
    it('muestra "Producción" cuando produccion=true', () => {
        renderComponente({
            bloques: [{ id_bloque: 1, peso_gr: '100', contenedor: 'Bolsa', produccion: true, contaminado: 0 }],
        })
        expect(screen.getAllByText('Producción').length).toBeGreaterThan(0)
    })

    it('muestra "Experimental" cuando produccion=false', () => {
        renderComponente({
            bloques: [{ id_bloque: 1, peso_gr: '100', contenedor: 'Bolsa', produccion: false, contaminado: 0 }],
        })
        expect(screen.getAllByText('Experimental').length).toBeGreaterThan(0)
    })
})

// Checkbox contaminado 

describe('TablaBloques — checkbox contaminado', () => {
    it('renderiza un checkbox por bloque', () => {
        renderComponente()
        const checkboxes = screen.getAllByRole('checkbox')
        // Hay vista móvil y desktop → 2 checkboxes por bloque
        expect(checkboxes.length).toBe(bloquesMock.length * 2)
    })

    it('el checkbox está marcado cuando contaminado=1', () => {
        renderComponente({
    ...
    it('el checkbox está desmarcado cuando contaminado=0', () => {
        renderComponente({
            bloques: [{ id_bloque: 1, peso_gr: '100', contenedor: 'Bolsa', produccion: 1, contaminado: 0 }],
        })
        const checkboxes = screen.getAllByRole('checkbox')
        checkboxes.forEach((cb) => expect(cb).not.toBeChecked())
    })

    it('el checkbox está marcado cuando contaminado=true', () => {
        renderComponente({
            bloques: [{ id_bloque: 1, peso_gr: '100', contenedor: 'Bolsa', produccion: 0, contaminado: true }],
        })
        const checkboxes = screen.getAllByRole('checkbox')
        checkboxes.forEach((cb) => expect(cb).toBeChecked())
    })

    it('el checkbox está desmarcado cuando contaminado=false', () => {
        renderComponente({
            bloques: [{ id_bloque: 1, peso_gr: '100', contenedor: 'Bolsa', produccion: 0, contaminado: false }],
        })
        const checkboxes = screen.getAllByRole('checkbox')
        checkboxes.forEach((cb) => expect(cb).not.toBeChecked())
    })
...
describe('TablaBloques — interacciones', () => {
    it('llama a onToggleContaminado con el id_bloque al hacer click en el checkbox', async () => {
    ...
    it('no lanza error al hacer click si onToggleContaminado no está definido', async () => {
        const user = userEvent.setup()
        renderComponente({
            bloques: [{ id_bloque: 1, peso_gr: '100', contenedor: 'Bolsa', produccion: 1, contaminado: 0 }],
            onToggleContaminado: undefined,
        })

        const checkboxes = screen.getAllByRole('checkbox')
        await expect(user.click(checkboxes[0])).resolves.not.toThrow()
    })

    it('llama a onToggleContaminado una sola vez por click', async () => {
        const user = userEvent.setup()
        const onToggleContaminado = vi.fn()
        renderComponente({
            bloques: [{ id_bloque: 3, peso_gr: '100', contenedor: 'Bolsa', produccion: 1, contaminado: 0 }],
            onToggleContaminado,
        })
        ...
        expect(onToggleContaminado).toHaveBeenCalledTimes(1)
    })
})